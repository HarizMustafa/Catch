package com.hariz.Catch 
{
	...
	public class Enemy extends EnemyTemplate
	{
		...
		public override function update():void
		{
			...
			if (distancesquared < 1200000) // 65000 = approximately 16 tiles
			{
				if (_player.x < x)
				{
					facing = RIGHT; // The sprite is facing the opposite direction than flixel is expecting, so hack it into the right direction
					acceleration.x = drag.x; //Mage AI goes left if player goes right
				}
				else if (_player.x > x)
				{
					facing = LEFT;
					acceleration.x = -drag.x; //goes right if player goes left
				}
				if (_player.y < y) { acceleration.y = -drag.y;} //I tried swapping the two drag.y's like I did the drag.x's above but it would result in the Mages going up into space....
